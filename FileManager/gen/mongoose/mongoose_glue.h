...
#define WIZARD_WIFI_NAME "MyNetwork"
#endif
...
void glue_start_delete(struct mg_str);  // Start an action
bool glue_check_delete(void);  // Check if action is still in progress

struct files {
  int size;
  char name[80];
};
bool glue_get_files(struct files *, size_t index);
void glue_set_files(struct files *, size_t index);
