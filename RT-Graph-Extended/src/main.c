...
static void myAccelSensor(struct accel *data){
	bno055_accel_float_t accelData;
	bno055_convert_float_accel_xyz_msq(&accelData);
	int d;

	d =   (int)(accelData.x * 100.0 + .5);
	data->x = (double)d/100.0;
	d =   (int)(accelData.y * 100.0 + .5);
	data->y =(double)d/100.0;
	d =   (int)(accelData.z * 100.0 + .5);
	data->z = (double)d/100.0;
}
...
int main(void) {
  ...
  mongoose_set_http_handlers("accel", myAccelSensor, NULL);
  mongoose_add_ws_reporter(500, "angular");
  mongoose_add_ws_reporter(500, "accel");
  for (;;) {
    mongoose_poll();
  }
